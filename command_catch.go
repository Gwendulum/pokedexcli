...
func commandCatch(cfg *config, userInput string) error {
	if userInput == "" {
		return fmt.Errorf("Please input a Pokemon")
	}
	...
	pokemon := Pokemon{}
	err := json.Unmarshal(page, &pokemon)
	if err != nil {
    	return err
	}
	fmt.Printf("Throwing a Pokeball at %s...\n", pokemon.Name)
	ChanceToCatch := pokemon.BaseExperience
	if rand.Intn(400) < ChanceToCatch {
		fmt.Printf("%s escaped!\n", pokemon.Name)
		} else {
		fmt.Printf("%s was caught!\n", pokemon.Name)
		
		_, exists := cfg.caughtPokemon[pokemon.Name]
		if exists {
